...
// fixed-point scale in the range 0..256, where 256 is 100%.
	...
	public:
		...
		void Print(const char* t, int x1, int y1, uint c, int scaling = 1);
		const int WhatIfPrintCenter(char* t, int scaling);
		void PrintCenter(char* t, int y1, uint c, int scaling = 1, int xOffset = 0);
		void Clear(uint c);
		void Line(float x1, float y1, float x2, float y2, uint c, bool dotted = false);
		void Plot(int x, int y, uint c);
		void LoadFromFile(const char* file);
		void CopyTo(Surface* dst, int x, int y);
		void Box(int x1, int y1, int x2, int y2, uint color, bool dotted = false);
