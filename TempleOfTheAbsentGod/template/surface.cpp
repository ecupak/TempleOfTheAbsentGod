...
void Surface::Box( int x1, int y1, int x2, int y2, uint c, bool dotted )
{
	Line( (float)x1, (float)y1, (float)x2, (float)y1, c, dotted );
	Line( (float)x2, (float)y1, (float)x2, (float)y2, c, dotted);
	Line( (float)x1, (float)y2, (float)x2, (float)y2, c, dotted);
	Line( (float)x1, (float)y1, (float)x1, (float)y2, c, dotted);
}
...
// [Credit] Alex 230453
// Print some text with the hard-coded mini-font.
void Surface::Print(const char* s, int x1, int y1, uint c, int scaling)
{
	// poor-man's clipping
	if (x1 < 0 || y1 < 0 || x1 + (strlen(s) * 6) >= width || y1 + 5 >= height)
	{
		return;
	}
	...
	for (int i = 0; i < (int)(strlen(s)); i++, t += 6 * scaling)
	{
		int pos = 0;
		if ((s[i] >= 'A') && (s[i] <= 'Z')) 
		{
			pos = transl[(unsigned short)(s[i] - ('A' - 'a'))];
		}
		else 
		{
			pos = transl[(unsigned short)s[i]];
		}
		
		uint* a = t;
		const char* u = (const char*)font[pos];

		for (int v = 0; v < 5; v++, u++, a += scaling * width)
		{
			for (int h = 0; h < 5 * scaling; h += scaling)
			{
				if (*u++ == 'o')
				{
					for (int w = 0; w < scaling; w++)
					{
...
// [Credit] Jacco, from the intake Tmpl8.
// Edited to work with scaling print.
const int Surface::WhatIfPrintCenter(char* t, int scaling)
{
	return (width - (int)strlen(t) * 6 * scaling) >> 1;
}

void Surface::PrintCenter(char* t, int y1, uint c, int scaling, int xOffset)
{
	int x = (width - (int)strlen(t) * 6 * scaling) >> 1;
	
	Print(t, x + xOffset, y1, c, scaling);
}
...
void Surface::Line( float x1, float y1, float x2, float y2, uint c, bool dotted )
{
